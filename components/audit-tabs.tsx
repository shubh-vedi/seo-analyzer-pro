...
import { AdvancedAiTip } from "@/lib/gemini"
import { Info, Code, Share2, Database, Link2, AlertCircle, Sparkles, Brain, CheckCircle2, AlertTriangle, XCircle, Network, Search, ShieldCheck } from "lucide-react"
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from "@/components/ui/accordion"
import { AEOResult } from "@/lib/aeo-scorer"
import { GEOResult } from "@/lib/geo-scorer"
import { AIOResult } from "@/lib/aio-scorer"
import { GuidelinesResult } from "@/lib/guidelines-scorer"
  ...
  return (
    <Tabs defaultValue={type === "text" ? "headings" : "meta"} className="w-full">
      <TabsList className="bg-slate-100/50 border border-slate-200/60 p-1 flex-wrap h-auto rounded-2xl gap-1 mb-8">
        ...
        <TabsTrigger value="geo" className="data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-xl px-4 py-2 text-xs font-bold text-slate-500 data-[state=active]:text-violet-600 transition-all flex items-center gap-2">
          <Network className="w-3.5 h-3.5" /> GEO Analysis
        </TabsTrigger>
        <TabsTrigger value="aio" className="data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-xl px-4 py-2 text-xs font-bold text-slate-500 data-[state=active]:text-amber-500 transition-all flex items-center gap-2">
          <Search className="w-3.5 h-3.5" /> AIO Analysis
        </TabsTrigger>
        <TabsTrigger value="guidelines" className="data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-xl px-4 py-2 text-xs font-bold text-slate-500 data-[state=active]:text-green-600 transition-all flex items-center gap-2">
      ...
      <TabsContent value="geo" className="animate-in fade-in slide-in-from-bottom-4 duration-500 outline-none">
        <div className="mb-8 p-8 rounded-[2rem] bg-linear-to-br from-violet-500 to-fuchsia-600 text-white shadow-xl shadow-violet-900/20">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <Network className="w-5 h-5 opacity-80" />
                <span className="text-[10px] font-black uppercase tracking-[0.3em] opacity-70">Generative Engine Optimization</span>
              </div>
              <h3 className="text-3xl font-black tracking-tight mb-2">GEO Visibility Grade</h3>
              <p className="text-violet-100 font-medium max-w-md">
                This score measures how well your content is structured for Retrieval-Augmented Generation (RAG) pipelines.
              </p>
            </div>
            <div className="text-center bg-white/10 backdrop-blur-md rounded-3xl p-8 border border-white/20 min-w-[160px]">
              <div className="text-6xl font-black mb-1">{data.geoResult?.grade || "F"}</div>
              <div className="text-[10px] font-black uppercase tracking-widest opacity-60">Composite Grade</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-4">
          {!data.geoResult ? (
            <div className="p-16 text-center bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-100">
              <p className="text-slate-400 font-bold">No GEO data available for this audit.</p>
            </div>
          ) : (
            data.geoResult.breakdown.map((item, i) => (
              <div key={i} className="bg-white border border-slate-100 rounded-[1.5rem] overflow-hidden shadow-sm hover:shadow-md transition-all">
                <div className="p-6 flex items-center justify-between gap-6">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                ...
                <div className="px-6 pb-2 pt-0 border-t border-slate-50">
                  <Accordion className="w-full">
                    <AccordionItem value="tip" className="border-0">
                      <AccordionTrigger className="py-3 text-[11px] font-black text-violet-600 uppercase tracking-widest hover:no-underline">
                        How to improve Visibility
                      </AccordionTrigger>
                      <AccordionContent className="text-slate-500 font-medium leading-relaxed pb-4">
                        {item.improveTip}
      ...
      <TabsContent value="aio" className="animate-in fade-in slide-in-from-bottom-4 duration-500 outline-none">
        <div className="mb-8 p-8 rounded-[2rem] bg-linear-to-br from-amber-500 to-orange-600 text-white shadow-xl shadow-amber-900/20">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <Search className="w-5 h-5 opacity-80" />
                <span className="text-[10px] font-black uppercase tracking-[0.3em] opacity-70">AI Overview Optimization</span>
              </div>
              <h3 className="text-3xl font-black tracking-tight mb-2">AIO Visibility Grade</h3>
              <p className="text-amber-100 font-medium max-w-md">
                This score measures how well your content is positioned to be featured in Google's AI Overviews.
              </p>
            </div>
            <div className="text-center bg-white/10 backdrop-blur-md rounded-3xl p-8 border border-white/20 min-w-[160px]">
              <div className="text-6xl font-black mb-1">{data.aioResult?.grade || "F"}</div>
              <div className="text-[10px] font-black uppercase tracking-widest opacity-60">Composite Grade</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-4">
          {!data.aioResult ? (
            <div className="p-16 text-center bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-100">
              <p className="text-slate-400 font-bold">No AIO data available for this audit.</p>
            </div>
          ) : (
            data.aioResult.breakdown.map((item, i) => (
              <div key={i} className="bg-white border border-slate-100 rounded-[1.5rem] overflow-hidden shadow-sm hover:shadow-md transition-all">
                <div className="p-6 flex items-center justify-between gap-6">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                ...
                <div className="px-6 pb-2 pt-0 border-t border-slate-50">
                  <Accordion className="w-full">
                    <AccordionItem value="tip" className="border-0">
                      <AccordionTrigger className="py-3 text-[11px] font-black text-amber-600 uppercase tracking-widest hover:no-underline">
                        How to improve Visibility
                      </AccordionTrigger>
                      <AccordionContent className="text-slate-500 font-medium leading-relaxed pb-4">
                        {item.improveTip}
      ...
      <TabsContent value="guidelines" className="animate-in fade-in slide-in-from-bottom-4 duration-500 outline-none">
        <div className="mb-8 p-8 rounded-[2rem] bg-linear-to-br from-green-500 to-emerald-600 text-white shadow-xl shadow-green-900/20">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div>
              <div className="flex items-center gap-3 mb-2">
                <ShieldCheck className="w-5 h-5 opacity-80" />
                <span className="text-[10px] font-black uppercase tracking-[0.3em] opacity-70">Search Quality Compliance</span>
              </div>
              <h3 className="text-3xl font-black tracking-tight mb-2">E-E-A-T Grade</h3>
              <p className="text-green-100 font-medium max-w-md">
                This score measures adherence to Google's Search Quality Rater Guidelines (Experience, Expertise, Authority, Trust).
              </p>
            </div>
            <div className="text-center bg-white/10 backdrop-blur-md rounded-3xl p-8 border border-white/20 min-w-[160px]">
              <div className="text-6xl font-black mb-1">{data.guidelinesResult?.grade || "F"}</div>
              <div className="text-[10px] font-black uppercase tracking-widest opacity-60">Composite Grade</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-4">
          {!data.guidelinesResult ? (
            <div className="p-16 text-center bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-100">
              <p className="text-slate-400 font-bold">No Guidelines data available for this audit.</p>
            </div>
          ) : (
            data.guidelinesResult.breakdown.map((item, i) => (
              <div key={i} className="bg-white border border-slate-100 rounded-[1.5rem] overflow-hidden shadow-sm hover:shadow-md transition-all">
                <div className="p-6 flex items-center justify-between gap-6">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                ...
                <div className="px-6 pb-2 pt-0 border-t border-slate-50">
                  <Accordion className="w-full">
                    <AccordionItem value="tip" className="border-0">
                      <AccordionTrigger className="py-3 text-[11px] font-black text-green-600 uppercase tracking-widest hover:no-underline">
                        How to improve Visibility
                      </AccordionTrigger>
                      <AccordionContent className="text-slate-500 font-medium leading-relaxed pb-4">
                        {item.improveTip}
